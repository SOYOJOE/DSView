...
void DecoderStack::do_decode_work()
{
    ...
    if (get_root_decoder_id() && strcmp(get_root_decoder_id(), "0:uart") == 0) {
        dsv_info("native uart skip: v3 direct text annotations");
        _progress = 100;
        _is_decoding = false;
        new_decode_data();
        decode_done();
...
bool DecoderStack::push_direct_annotation(uint64_t start_sample, uint64_t end_sample,
                                          int format, int type,
                                          const std::vector<QString> &annotations)
{
    const bool ok = push_native_annotation(start_sample, end_sample, format, type,
                                           annotations);
    if (ok && (_result_count == 1 || (_result_count & 0xff) == 0))
        new_decode_data();
    return ok;
}
